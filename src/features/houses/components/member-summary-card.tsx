		...
		<div className='flex items-start justify-between'>
			<div className='flex flex-col items-start'>
				...
				{transferOrganizer.isSuccess ? (
					...
					<>
						<Button variant='split_ghost' className='!text-xs md:!text-base' onClick={handleDeleteClick}>
							{`delete, ${member.firstName}`}
						</Button>
						<Button variant='split_ghost' className='!text-xs md:!text-base' onClick={handleTransferClick}>
					...
					<div className='grid w-full grid-cols-2'>
						{member.owedExpenses.map(expense => (
							<Fragment key={expense.expenseId}>
								<div className='text-right text-xs text-foreground md:text-base'>{expense.category.toLowerCase()}</div>
