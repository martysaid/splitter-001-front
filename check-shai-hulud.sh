...
# =============================================================================
# Shai-Hulud v2 NPM Supply Chain Attack Detection Script
# =============================================================================
# Checks package.json and package-lock.json for packages compromised in the
# November 2025 "Shai-Hulud: The Second Coming" supply chain attack.
#
# Usage: ./check-shai-hulud.sh [directory]
#        If no directory specified, uses current directory
#
# Exit codes:
#   0 - No infected packages found
#   1 - Infected packages detected
...
RESULTS_FILE=$(mktemp)
trap 'rm -f "$INFECTED_LIST" "$RESULTS_FILE"' EXIT

# =============================================================================
# Infected packages database
# Format: package_name:version1,version2,version3
# =============================================================================
generate_infected_list() {
    cat << 'INFECTED_EOF'
...
version_matches() {
    ...
    local infected_versions="$2"
    local normalized
    normalized=$(normalize_version "$installed_version")
    
    IFS=',' read -ra VERSIONS <<< "$infected_versions"
    for infected_ver in "${VERSIONS[@]}"; do
        if [[ "$normalized" == "$infected_ver" ]]; then
            return 0
        fi
    done
    return 1
...
check_package_json() {
    ...
    log_info "Scanning: $pkg_file"
    
    if ! python3 -c "import json; json.load(open('$pkg_file'))" 2>/dev/null; then
        if ! command_exists jq || ! jq empty "$pkg_file" 2>/dev/null; then
    ...
    local deps
    deps=$(PKG_FILE="$pkg_file" python3 << 'PYEOF'
import json
import sys
import os

try:
    pkg_file = os.environ.get("PKG_FILE", "")
...
PYEOF
    ...
    if [[ $? -ne 0 ]]; then
        log_error "Failed to parse $pkg_file"
        return 2
    fi
    ...
    while IFS=':' read -r pkg_name pkg_version; do
        ...
        local infected_entry
        infected_entry=$(grep -E "^$(escape_for_grep "$pkg_name"):" "$INFECTED_LIST" 2>/dev/null || true)
        
        if [[ -n "$infected_entry" ]]; then
            local infected_versions="${infected_entry#*:}"
            local normalized_version
            normalized_version=$(normalize_version "$pkg_version")
            
            if version_matches "$pkg_version" "$infected_versions"; then
                log_critical "INFECTED: $pkg_name@$normalized_version in $pkg_file"
                echo "INFECTED|$pkg_file|$pkg_name|$normalized_version|direct" >> "$RESULTS_FILE"
                ((INFECTED_COUNT++)) || true
                found=1
            else
                log_warning "Package $pkg_name is in infected list (versions: $infected_versions), your version: $normalized_version"
                echo "WARNING|$pkg_file|$pkg_name|$normalized_version|direct|infected_versions:$infected_versions" >> "$RESULTS_FILE"
            fi
        fi
    done <<< "$deps"
...
check_package_lock() {
    ...
    log_info "Scanning: $lock_file (full dependency tree)"
    
    local deps
    deps=$(LOCK_FILE="$lock_file" python3 << 'PYEOF'
import json
import sys
import os
...
try:
    ...
    if "dependencies" in lock and isinstance(lock["dependencies"], dict):
...
PYEOF
    ...
    if [[ $? -ne 0 ]]; then
        log_error "Failed to parse $lock_file"
        return 2
    fi
    ...
    while IFS=':' read -r pkg_name pkg_version; do
        ...
        if [[ -n "$infected_entry" ]]; then
            ...
            if version_matches "$pkg_version" "$infected_versions"; then
                log_critical "INFECTED: $pkg_name@$pkg_version in $lock_file (transitive dependency)"
                echo "INFECTED|$lock_file|$pkg_name|$pkg_version|transitive" >> "$RESULTS_FILE"
                ((INFECTED_COUNT++)) || true
                found=1
            fi
        fi
...
check_yarn_lock() {
    ...
    log_info "Scanning: $lock_file (yarn lockfile)"
...
try:
    ...
    packages = set()
    pattern = r'^"?(@?[^@\s"]+)@'
    version_pattern = r'^\s+version:?\s+"?([^"\s]+)"?\s*$'
    
    lines = content.split("\n")
    current_pkg = None
    
    for line in lines:
        pkg_match = re.match(pattern, line)
        if pkg_match:
            current_pkg = pkg_match.group(1)
            continue
        
        if current_pkg:
            ver_match = re.match(version_pattern, line)
            if ver_match:
                version = ver_match.group(1)
                packages.add(f"{current_pkg}:{version}")
                current_pkg = None
...
PYEOF
    ...
    while IFS=':' read -r pkg_name pkg_version; do
        ...
        if [[ -n "$infected_entry" ]]; then
            ...
            if version_matches "$pkg_version" "$infected_versions"; then
                log_critical "INFECTED: $pkg_name@$pkg_version in $lock_file (yarn dependency)"
                echo "INFECTED|$lock_file|$pkg_name|$pkg_version|yarn" >> "$RESULTS_FILE"
                ((INFECTED_COUNT++)) || true
                found=1
            fi
        fi
...
check_shai_hulud_indicators() {
    local dir="$1"
    local found=0
    
    log_info "Checking for Shai-Hulud malware indicators..."
    ...
    for file in "${malicious_files[@]}"; do
        if [[ -f "$dir/$file" ]]; then
            log_critical "MALWARE INDICATOR: Found suspicious file: $dir/$file"
            echo "MALWARE|$dir/$file|suspicious_file" >> "$RESULTS_FILE"
            found=1
        fi
    done
    
    if [[ -d "$dir/.git" ]]; then
        if git -C "$dir" branch -a 2>/dev/null | grep -qi "shai-hulud"; then
            log_critical "MALWARE INDICATOR: Found shai-hulud branch in git repository"
            echo "MALWARE|$dir/.git|shai_hulud_branch" >> "$RESULTS_FILE"
            found=1
        fi
    fi
    
    if [[ -d "$dir/.github/workflows" ]]; then
        if grep -rEli "Buffer\.from.*base64|data\.json|Shai.?Hulud|webhook\.site" "$dir/.github/workflows" 2>/dev/null; then
            log_critical "MALWARE INDICATOR: Suspicious patterns found in GitHub workflows"
            echo "MALWARE|$dir/.github/workflows|suspicious_workflow_content" >> "$RESULTS_FILE"
            found=1
        fi
    fi
    
    if [[ -d "$dir/node_modules" ]]; then
        local suspicious_in_modules
        suspicious_in_modules=$(find "$dir/node_modules" -maxdepth 3 \( -name "setup_bun.js" -o -name "bun_environment.js" \) 2>/dev/null | head -5)
        if [[ -n "$suspicious_in_modules" ]]; then
            log_critical "MALWARE INDICATOR: Suspicious Shai-Hulud payload files found in node_modules"
            echo "MALWARE|$dir/node_modules|suspicious_payload_files" >> "$RESULTS_FILE"
            found=1
        fi
    fi
...
check_preinstall_scripts() {
    ...
    local scripts
    scripts=$(PKG_FILE="$pkg_file" python3 << 'PYEOF'
import json
import os
...
try:
    ...
    if "scripts" in pkg:
        for script_name in ["preinstall", "postinstall", "prepare"]:
            if script_name in pkg["scripts"]:
                script_content = pkg["scripts"][script_name]
                suspicious = ["bun ", "setup_bun", "bun_environment", "base64", 
                              "webhook.site", "eval(", "Function(", "Buffer.from"]
                for pattern in suspicious:
                    if pattern.lower() in script_content.lower():
                        print(f"{script_name}:{script_content[:100]}")
                        break
...
PYEOF
    ...
    if [[ -n "$scripts" ]]; then
        while IFS=':' read -r script_name script_content; do
            log_warning "Suspicious $script_name script in $pkg_file: ${script_content:0:50}..."
            echo "WARNING|$pkg_file|suspicious_script|$script_name" >> "$RESULTS_FILE"
        done <<< "$scripts"
    fi
}
...
main() {
    echo ""
    echo -e "${BOLD}======================================================================${NC}"
    echo -e "${BOLD}     Shai-Hulud v2 NPM Supply Chain Attack Detection Script${NC}"
    echo -e "${BOLD}                     November 2025 Outbreak${NC}"
    echo -e "${BOLD}======================================================================${NC}"
    echo ""
    ...
    log_info "Loading infected packages database (700+ packages)..."
    generate_infected_list > "$INFECTED_LIST"
    local total_infected
    total_infected=$(wc -l < "$INFECTED_LIST")
    log_info "Loaded $total_infected infected package entries"
    echo ""
    ...
    while IFS= read -r -d '' file; do
        lock_files+=("$file")
    done < <(find "$TARGET_DIR" -name "package-lock.json" -not -path "*/node_modules/*" -print0 2>/dev/null)
    
    while IFS= read -r -d '' file; do
        yarn_files+=("$file")
    done < <(find "$TARGET_DIR" -name "yarn.lock" -not -path "*/node_modules/*" -print0 2>/dev/null)
    
    if [[ ${#pkg_files[@]} -eq 0 && ${#lock_files[@]} -eq 0 && ${#yarn_files[@]} -eq 0 ]]; then
        log_warning "No package.json, package-lock.json, or yarn.lock files found in $TARGET_DIR"
        exit 0
    fi
    
    log_info "Found ${#pkg_files[@]} package.json, ${#lock_files[@]} package-lock.json, ${#yarn_files[@]} yarn.lock file(s)"
    echo ""
    
    echo -e "${BOLD}--- Scanning package.json files ---${NC}"
    for pkg_file in "${pkg_files[@]}"; do
        check_package_json "$pkg_file" || true
        check_preinstall_scripts "$pkg_file"
        echo ""
    done
    
    echo -e "${BOLD}--- Scanning package-lock.json files ---${NC}"
    for lock_file in "${lock_files[@]}"; do
        check_package_lock "$lock_file" || true
        echo ""
    done
    
    if [[ ${#yarn_files[@]} -gt 0 ]]; then
        echo -e "${BOLD}--- Scanning yarn.lock files ---${NC}"
        for yarn_file in "${yarn_files[@]}"; do
            check_yarn_lock "$yarn_file" || true
            echo ""
        done
    fi
    
    echo -e "${BOLD}--- Checking for malware indicators ---${NC}"
    check_shai_hulud_indicators "$TARGET_DIR" || true
    echo ""
    ...
    echo -e "${BOLD}======================================================================${NC}"
    echo ""
    echo -e "  Packages checked:    ${CYAN}$CHECKED_PACKAGES${NC}"
    echo -e "  Warnings:            ${YELLOW}$WARNINGS${NC}"
    
    if [[ $INFECTED_COUNT -gt 0 ]]; then
        echo -e "  ${RED}${BOLD}INFECTED PACKAGES:   $INFECTED_COUNT${NC}"
        echo ""
        echo -e "${RED}${BOLD}WARNING: Infected packages detected!${NC}"
        echo ""
        echo "Recommended actions:"
        echo "  1. DO NOT run npm install or npm ci until packages are updated"
        echo "  2. Pin dependencies to known clean versions (pre-November 21, 2025)"
        echo "  3. Revoke and regenerate:"
        echo "     - npm tokens"
        echo "     - GitHub PATs and SSH keys"  
        echo "     - AWS/Azure/GCP credentials"
        echo "  4. Check for unauthorized GitHub repos named Shai-Hulud"
        echo "  5. Audit GitHub Actions workflows for suspicious commits"
        echo "  6. Review CI/CD pipelines for unauthorized changes"
        echo ""
        
        echo -e "${BOLD}Infected packages found:${NC}"
        grep "^INFECTED" "$RESULTS_FILE" | while IFS='|' read -r status file pkg version dep_type; do
            echo -e "  ${RED}*${NC} $pkg@$version ($dep_type in $file)"
        done
        
        exit 1
    else
        echo -e "  ${GREEN}${BOLD}INFECTED PACKAGES:   0${NC}"
        echo ""
        echo -e "${GREEN}No infected packages detected!${NC}"
        echo ""
        
        if [[ $WARNINGS -gt 0 ]]; then
            echo -e "${YELLOW}Note: Some packages in your project exist in the infected list"
            echo -e "but are at different (likely safe) versions. Review warnings above.${NC}"
            echo ""
        fi
        
        echo "Stay safe:"
        echo "  - Keep dependencies updated to latest clean versions"
        echo "  - Enable npm audit in your CI/CD pipeline"
        echo "  - Use lockfiles and verify integrity"
